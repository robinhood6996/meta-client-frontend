...
const InactiveEscortsList = ({className}) => {
  ...
  useEffect(() => {
    if (!search) {
      navigate(`/inactive-escorts?limit=${limit}&offset=0`)
    } else {
  ...
  //api call
  const {data, isFetching, isError, isSuccess} = useGetInactiveEscortsQuery(query, {skip: !query})
  const [
    deleteEscort,
  ...
  const handleSearch = (e) => {
    ...
    if (searchText?.length > 0) {
      navigate(`/inactive-escorts?limit=${limit}&offset=0&search=${searchText}`)
    } else {
      navigate(`/inactive-escorts?limit=${limit}&offset=0`)
    }
  }
