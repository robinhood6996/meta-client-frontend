...
const VerifiedIDList = ({className}) => {
  ...
  useEffect(() => {
    if (!search) {
      navigate(`/active-verification?limit=${limit}&offset=0`)
    } else {
  ...
  const handleSearch = (e) => {
    ...
    if (searchText?.length > 0) {
      navigate(`/active-verification?limit=${limit}&offset=0&search=${searchText}`)
    } else {
      navigate(`/active-verification?limit=${limit}&offset=0`)
    }
  }
                ...
                {isFetching ? (
                  ...
                  <>
                    {data?.data?.length > 0 ? (
                      ...
                      <NotFoundComponent type='Verified' />
                    )}
                  </>
                ) : (
                  <ErrorComponent />
