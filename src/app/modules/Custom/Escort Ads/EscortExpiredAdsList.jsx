...
const EscortExpiredAdsList = ({className}) => {
  ...
  useEffect(() => {
    if (!search) {
      navigate(`/expired-escort-ads?isPaid=true&expired=true&limit=${limit}&offset=0`)
    } else {
  ...
  const handleSearch = (e) => {
    ...
    if (searchText?.length > 0) {
      navigate(
        `/expired-escort-ads?isPaid=true&expired=true&limit=${limit}&offset=0&search=${searchText}`
      )
    } else {
      navigate(`/expired-escort-ads?isPaid=true&expired=true&limit=${limit}&offset=0`)
    }
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value !== 'default') {
                    ...
                    navigate(`/expired-escort-ads?${objectToParam(oldQ)}`)
                    return oldQ
                  } else {
                    delete oldQ.payment
                    navigate(`/expired-escort-ads?${objectToParam(oldQ)}`)
